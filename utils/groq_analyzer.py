...
def analyze_report_with_groq(report_text):
    ...
    # NEW SCHEMA: Instruct Llama to generate detailed explanations and expected outcomes for each prompt.
...
{{
  ...
  "strategic_gaps": [
    {{
      ...
      "detailed_explanation": "2-3 sentences explaining exactly why this is a problem for their specific goals.",
      "option_1_id": Integer matching the best prompt ID from the manifest,
      "option_1_expected_outcome": "1-2 sentences explaining exactly what this prompt will do to fix the gap.",
      "option_2_id": Integer matching the second best prompt ID alternative,
      "option_2_expected_outcome": "1-2 sentences explaining how this alternative prompt fixes the gap using a different approach."
    }}
  ]
}}
Note: Provide ALL critical strategic gaps found in the report (typically between 1 and 6 gaps). Do not force exactly 3.
"""
