...
def render_step5():
    ...
    report = st.session_state.get("pasted_report", "")
    
    # 1. Parse Metrics directly from the raw report
    score_match = re.search(r'PROFILE_HEALTH_SCORE:\s*(\d+)', report)
    
    # Updated regex to handle the new ALL_CRITICAL_GAPS heading (and backwards compatible with TOP_3)
    gaps_match = re.search(r'(?:ALL_CRITICAL_GAPS|TOP_3_CRITICAL_GAPS|CRITICAL_GAPS):\s*(.*?)(?=\n###|\n\n|$)', report, re.DOTALL | re.IGNORECASE)
    
    health_score = score_match.group(1) if score_match else "N/A"
    critical_gaps = gaps_match.group(1).strip() if gaps_match else "Data not found in correct format."

    executive_summary = groq_data.get("executive_summary", "No summary provided.")
    strategic_gaps = groq_data.get("strategic_gaps", [])
    
    # 2. Display Groq's Smart Metrics
    st.info(f"**Profile Health Score:** {health_score}/100")
    st.write(f"**Executive Summary:** {executive_summary}")
    
    # Show the raw list of all gaps found by the first AI
    st.warning(f"**All Critical Gaps Identified:**\n\n{critical_gaps}")
    
    st.write("---")
    st.subheader("📋 Your Personalized Optimization Strategy")
    st.write("Review the identified gaps below. For each gap, read the strategy and click an expander option to view and copy the execution prompt.")
    st.write("---")
    
    # 3. BULLETPROOF ID MATCHING & UI RENDERING
    all_prompts = load_prompt_library()
    prompt_dict = {str(p.get("id")): p for p in all_prompts}
    ...
    if strategic_gaps:
        for index, gap in enumerate(strategic_gaps, 1):
            ...
            detailed_explanation = gap.get("detailed_explanation", "")
            
            opt_1_outcome = gap.get("option_1_expected_outcome", "This prompt will optimize this section of your profile.")
            opt_2_outcome = gap.get("option_2_expected_outcome", "This alternative prompt takes a different approach to fix the gap.")
            
            gaps_for_pdf.append(f"{gap_name}: {detailed_explanation}")
            
            st.markdown(f"### 🎯 Gap {index}: {gap_name}")
            st.markdown(f"**The Problem:** {detailed_explanation}")
            st.write("") 
            ...
            with col1:
                if opt_1:
                    ...
                    st.markdown(f"**Strategy Option A: {opt_1.get('title')}**")
                    st.markdown(f"_{opt_1_outcome}_")
                    
                    with st.expander("👁️ View & Copy Prompt", expanded=False):
                        st.caption("Copy this prompt and paste it into your external LLM:")
            ...
            with col2:
                if opt_2:
                    ...
                    st.markdown(f"**Strategy Option B: {opt_2.get('title')}**")
                    st.markdown(f"_{opt_2_outcome}_")
    ...
    st.success(
        """
        **What to do next:**
        1. **Choose:** Pick either Option A or Option B for each gap based on your goals.
        2. **Execute:** Open the chosen expander, click the copy icon on the top right of the code block, and paste it into your AI tool (Claude, ChatGPT, etc.).
        3. **Apply:** Take the AI's generated copy adjustments and apply them to your live LinkedIn profile.
        4. **Re-Test:** Click **Start Over** below, run a fresh audit, and watch your Profile Health Score climb!
        """
    )
